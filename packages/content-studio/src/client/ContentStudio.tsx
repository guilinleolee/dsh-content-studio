...
/** The three top-level workbench views. */
type StudioView = 'create' | 'library' | 'calendar'
    ...
    <div className={css.surface} role="dialog" aria-modal="true" aria-label={t('studio.title')}>
      <div className={css.frame}>
        <header className={css.header}>
          <div>
            <h1 className={css.title}>{t('studio.title')}</h1>
            <p className={css.subtitle}>{t('studio.subtitle')}</p>
          </div>
          <div className={css.headerControls}>
            <div className={css.views} role="tablist">
              {(['create', 'library', 'calendar'] as const).map(candidate => (
                <button
                  key={candidate}
                  type="button"
                  role="tab"
                  aria-selected={view === candidate}
                  className={clsx(css.tab, view === candidate && css.tabActive)}
                  onClick={() => { setView(candidate) }}
                >
                  {t(candidate === 'create' ? 'view.create' : candidate === 'library' ? 'view.library' : 'view.calendar')}
                </button>
              ))}
            </div>
            <button
