...
export function normalizeInput(input) {
    if (typeof input.title !== 'string' || input.title.trim().length === 0)
        return { detail: 'title must be a non-empty string' };
    if (typeof input.date !== 'string' || !DATE_PATTERN.test(input.date))
        return { detail: 'date must be YYYY-MM-DD' };
    if (input.time !== null && (typeof input.time !== 'string' || !isValidTime(input.time)))
        return { detail: 'time must be HH:mm (00:00–23:59) or null' };
    if (!STATUSES.includes(input.status))
        return { detail: 'unknown status' };
    if (!KINDS.includes(input.kind))
        return { detail: 'unknown kind' };
    return {
        item: {
 ...
 */
export async function readSchedule(file) {
    let raw;
    try {
    ...
    if (root.formatVersion !== 0)
        return { file, items: [], problems: [`unsupported calendar formatVersion ${String(root.formatVersion)}`] };
    if (!Array.isArray(root.items))
        return { file, items: [], problems: ['calendar file has no items array'] };
    const items = [];
    const problems = [];
    for (const entry of root.items) {
        if (isItem(entry))
            ...
            problems.push(`dropped one invalid calendar record: ${JSON.stringify(entry).slice(0, 120)}`);
    }
 ...
 */
export async function mutateSchedule(file, mutate) {
