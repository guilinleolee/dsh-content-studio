...
import type {
  ContentOutputsSnapshot, OutputKind, OutputMetadata, OutputProject, OutputProjectProblem, OutputStatus,
} from './types.ts'
...
const KINDS: readonly OutputKind[] = ['article', 'xhs-note', 'video', 'cards', 'poster', 'audio', 'other']
const STATUSES: readonly OutputStatus[] = ['draft', 'ready', 'published']

function isProjectEntry(name: string, type: 'file' | 'directory' | 'other'): boolean {
 ...
 */
export function parseMetadata(raw: string): OutputMetadata | undefined {
  let value: unknown
  try {
  ...
  if (record.formatVersion !== 0) return undefined
  if (typeof record.title !== 'string' || record.title.length === 0) return undefined
  if (!KINDS.includes(record.kind as OutputKind)) return undefined
  if (record.platform !== null && typeof record.platform !== 'string') return undefined
  if (!STATUSES.includes(record.status as OutputStatus)) return undefined
  if (!Array.isArray(record.tags) || !record.tags.every(tag => typeof tag === 'string')) return undefined
  if (record.summary !== null && typeof record.summary !== 'string') return undefined
  return value as OutputMetadata
}
...
export async function scanProject(
  root: string,
  topic: string,
): Promise<{ project?: OutputProject; problem?: string }> {
  ...
  const project: OutputProject = {
    topic: topic as OutputProject['topic'],
 ...
 */
export async function scanOutputs(root: string): Promise<ContentOutputsSnapshot> {
  let entries
  try {
  ...
  const topics = entries
    .map(entry => ({ name: entry.name, type: entry.isDirectory() ? 'directory' as const : entry.isFile() ? 'file' as const : 'other' as const }))
    .filter(entry => isProjectEntry(entry.name, entry.type))
    .map(entry => entry.name)
    .sort()

  const projects: OutputProject[] = []
  const problems: OutputProjectProblem[] = []
  for (const topic of topics) {
    const scanned = await scanProject(root, topic)
    if (scanned.project !== undefined) projects.push(scanned.project)
    else if (scanned.problem !== undefined) problems.push({ topic, detail: scanned.problem })
  }
  return { root, projects, problems }
}
